## Eviter la lecture automatique des vidéos et des sons
...
D'autre part, même en n'activant pas l'autoplay, certaines parties de la vidéo ou du fichier audio peuvent être téléchargées. 
Si l'utilisateur ne lance pas la lecture de la vidéo, ces données seront téléchargées inutilement. 
Il convient alors de forcer le navigateur de ne rien précharger, en définissant la valeur de l'attribut `preload` des balises `<video>` ou `<audio> à `none`.
...
|-------------------|:-------------------------:|
| de balise `<vidéo>` ou `<audio>` avec un attribut `preload="none"` ou `autoplay` | 0 |
