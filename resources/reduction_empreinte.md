...
-	*Scaphandre* est un agent qui mesure la consommation d’énergie d’un serveur en temps réel et en production. Il se base sur les métriques système et s’intègre dans les infrastructures physiques et virtualisées. Il dispose d’une intégration à des outils de remontée d’alerte et de représentation graphique. C’est un outil gratuit et open-source. (https://github.com/hubblo-org/scaphandre)
-	*Power API* est également un agent qui mesure la consommation d’énergie d’un serveur en temps réel et en production. Il se base sur les métriques système, et a été développé par l’université de Caen. C’est un outil gratuit et open-source (http://powerapi.org/).
