---
refID: '0015'
title: >-
  Utilizar sólo las partes esenciales de las bibliotecas JavaScript y frameworks
  CSS
createdAt: 2024-02-14T11:31:18.389Z
updatedAt: 2025-11-27T14:41:08.717Z
language: es
published: true
refType: RWEB
versions:
  - version: GreenIT
    idRef: 0811
  - version: 2.0.0
    idRef: '0077'
  - version: 3.0.0
    idRef: '0040'
  - version: 4.0.0
    idRef: '0015'
  - version: 5.0.0
    idRef: '0015'
people: TBD
responsible:
  - responsible: src/content/personas/es/arquitecto-software-desarrollador.mdx
lifecycle: 2-concept
rgesn: '6.5'
environmental_impact: 4
priority_implementation: 5
moe: 4
tiers: network
saved_resources:
  - cpu
  - ram
  - network
validations:
  - rule: de librerías en las que se utilicen partes no indispensables
    maxValue: '"1"'
---

## Descripción

Bibliotecas JavaScript como jQuery y frameworks CSS listos para usar (Bootstrap, skeleton, Gumby, foundation...) son herramientas excelentes para realizar sitios rápidamente, ya que satisfacen casi todas las necesidades más comunes.
Por otro lado, por lo general solo se utiliza una pequeña parte de ellos; pero estos marcos y bibliotecas no se basan todos en una arquitectura modular, lo que obliga al usuario a descargar toda la biblioteca para utilizar solo un pequeño porcentaje de sus funcionalidades.

En la medida de lo posible, es preferible prescindir de estas bibliotecas (véase [https://youmightnotneedjquery.com](https://youmightnotneedjquery.com)) o conservar únicamente las porciones realmente utilizadas (véase [https://getbootstrap.com/customize](https://getbootstrap.com/customize)).

Utilizar un Bundler (ej: Webpack) permite hacer fácilmente el tree shaking, es decir eliminar del código "muerto" entonces no utilizado.

## Ejemplo

Algunos frameworks, aquí Bootstrap, permiten crear bibliotecas a medida que contienen solo las porciones realmente utilizados por el sitio, lo que reduce el peso de las bibliotecas y los recursos consumidos durante la ejecución.
